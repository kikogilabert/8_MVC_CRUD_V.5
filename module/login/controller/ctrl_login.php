...
switch ($_GET['op']) {
    case 'list';
    // echo json_encode('hola list');
    // exit;
        include("module/login/view/login-register.html");
        // include('view/pages/footer.html');
        break;
    ...
    case 'register':
        ...
        try {
            $daoLog = new DAOLogin();
            $check2 = $daoLog->select_username($_POST['username_reg']);
        } catch (Exception $e) {
            echo json_encode("error");
            exit;
        }

        if ($check) { //esto es que son iguales a alguno en la BBDD
            $check_email = false;
        }else {
            $check_email = true;
        }
